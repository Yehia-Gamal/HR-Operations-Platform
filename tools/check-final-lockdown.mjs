...
assert(read('shared/js/supabase-config.js').includes('full-workflow-live-20260504'), 'Supabase config cache version must be final lockdown.');
assert(read('sw.js').includes('hr-attendance-full-workflow-live-20260504'), 'Service Worker cache must be final lockdown.');
assert(read('shared/js/register-sw.js').includes('hr-attendance-full-workflow-live-20260504'), 'Service Worker registration cache must match final lockdown.');
assert(read('shared/js/api.js').includes('034a_final_lockdown_cleanup.sql'), 'API migration list must include Patch 034.');
...
assert(read('shared/js/api.js').includes('expectedPatch: "064_attendance_fallback_workflow.sql"'), 'Expected patch must be 043.');
